...
var interRoute = require("./interRoute").interRoute;
var handleRoute = require("./interRoute").handleRoute;
//Public API
module.exports = {
...
function loadRouteObj(app, prefix, file, options) {
    ...
    for (var method in routeObj.prototype) {
        ...
        callargs.push(method);
        callargs.push(prefix);
        callargs.push(routeObj);
        //bind express app
        options.logger.debug("creating endpoint: " + prefix + method);
        // adding Route to the inter Route
        interRoute.addRoute(callargs);
    }
    //add all route
    app['all'].apply(app,['*',handleRoute]);
}
